...
import { 
  ...
  MapPin, Building2, CheckCircle2, Ban, Plus, 
  FileText, Activity, CheckSquare
} from 'lucide-react';
...
const CFADetail = () => {
  ...
  const fetchData = async () => {
    try {
      const { data } = await api.get(`/admin/cfa-master/${id}`);
      setCFA(data.data.cfa);
      setUsers(data.data.users || []);
      setStats(data.data.stats || { complaintCount: 0, activeComplaints: 0, completedComplaints: 0 });
    } catch (e) {
      toast.error('Failed to load CFA details');
      navigate('/cfa-management');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateUser = async () => {
    ...
    setSaving(true);
    try {
      await api.post(`/admin/cfa-master/${id}/users`, subForm);
    ...
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button 
            onClick={() => navigate('/cfa-management')} 
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-bold text-gray-900">{cfa.company || 'CFA Detail'}</h1>
              <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${cfa.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                {cfa.isActive ? 'Active' : 'Inactive'}
              </span>
            </div>
            <p className="text-sm text-gray-500 flex items-center gap-1">
              <Building2 size={14} /> Sales Office Code: <span className="font-bold text-blue-600">{cfa.code}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
              <MapPin size={18} className="text-blue-500" /> Location Details
            </h3>
            <div className="space-y-4">
              <div>
                <p className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Region / Area</p>
                <p className="text-gray-700 font-medium mt-1">{cfa.region || '—'}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Country</p>
                <p className="text-gray-700 font-medium mt-1">{cfa.country || 'India'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <h3 className="font-bold text-gray-900 px-1">Complaint Overview</h3>
            <div className="grid grid-cols-1 gap-3">
              <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-tight">Total Complaints</p>
                </div>
              </div>
              <div className="bg-white p-4 rounded-2xl border border-blue-100 shadow-sm flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-black text-blue-600">{stats.activeComplaints}</p>
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-tight text-blue-600/60">Active Pending</p>
                </div>
              </div>
              <div className="bg-white p-4 rounded-2xl border border-green-100 shadow-sm flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-tight text-green-600/60">Completed / Closed</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="p-6 border-b border-gray-50 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">Manage users who can access this CFA entity</p>
              </div>
              <button 
            ...
            <div className="divide-y divide-gray-50">
              {users.length === 0 ? (
                <div className="p-12 text-center">
                  ...
                  <p className="text-gray-400 text-sm italic">No staff accounts created for this sales office</p>
                </div>
              ) : (
                users.map(user => (
                  <div key={user._id} className="p-4 hover:bg-gray-50/50 transition-colors flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{user.email} • <span className="font-medium text-blue-600">{user.roleLabel || 'CFA Staff'}</span></p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
          ...
          <div className="bg-blue-50/50 rounded-2xl p-6 border border-blue-100">
             <h4 className="text-xs font-bold text-blue-700 uppercase tracking-widest mb-2">Entity Status</h4>
             <p className="text-sm text-blue-800 leading-relaxed font-medium">
               This CFA entity is currently <span className="font-bold underline">{cfa.isActive ? 'Active' : 'Deactivated'}</span>. 
               {cfa.isActive 
                 ? " It can process pickups and receive complaints at its designated sales office." 
                 : " All associated accounts will be unable to log in until the entity is reactivated."}
             </p>
          </div>
        </div>
      </div>
      ...
      {/* Add User Modal */}
      <Modal isOpen={showAddUser} onClose={() => setShowAddUser(false)} title="Create CFA Staff Account">
        <div className="space-y-4 pt-2">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-gray-400 uppercase px-1">Login Email</label>
              <input 
                value={subForm.email} 
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-gray-400 uppercase px-1">Role / Designation</label>
            <input 
